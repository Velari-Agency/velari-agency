...
(function () {
  ...
  // ─── Page detection ──────────────────────────────────────────
  // GitHub Pages strips .html from URLs, so normalize to slug
  var path = window.location.pathname;
  var page = path.split('/').pop() || 'index';
  if (page === '' || page === '/') page = 'index';
  page = page.replace('.html', '');  // "content-engine.html" → "content-engine"
  var pageSlug = page;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (e) {
      ...
      if (page === 'index' && href.indexOf('contact') !== -1) {
        fireMeta('Schedule', {
      ...
      if (page !== 'content-engine' && href.indexOf('content-engine') !== -1) {
